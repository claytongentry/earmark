defmodule Earmark.HtmlRenderer do
  ...
  def link(url, text, _context),        do: ~s[<a href="#{url}">#{text}</a>]
  def link(url, text, nil, _context),   do: ~s[<a href="#{url}">#{text}</a>]
  def link(url, text, title, _context), do: ~s[<a href="#{url}" title="#{title}">#{text}</a>]
